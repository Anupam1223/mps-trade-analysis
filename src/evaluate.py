...
def plot_prediction_intervals(y_true, y_preds, n=100):
    ...
    # FIX: Conditionally plot the median prediction only if it exists
    if 0.5 in y_preds:
    ...
    plt.fill_between(idx, y_preds[lower_q][:n], y_preds[upper_q][:n],
                     color='skyblue', alpha=0.4, label=f'{lower_q*100:.0f}-{upper_q*100:.0f}% Interval')
    plt.title('Prediction Intervals vs. True Values (First 100 Points)')
...
def coverage_analysis(y_true, y_preds):
    ...
    plt.title('Quantile Coverage Calibration Plot')
    plt.legend()
    plt.grid(True)
    plt.show()


# ... (plot_training_history, calculate_pinball_loss, etc. remain the same) ...
...
def scatter_pred_vs_true(y_true, y_pred, quantile):
    plt.figure(figsize=(6, 6))
    plt.scatter(y_true, y_pred, alpha=0.5, s=10)
    plt.plot([y_true.min(), y_true.max()], [y_true.min(), y_true.max()], 'r--')
    plt.title(f'Predicted vs Actual (Quantile={quantile})')
    plt.xlabel('Actual')
...
def plot_prediction_interval_width(preds_dict):
...
def plot_error_distribution(y_true, y_pred):
    ...
    plt.title('Prediction Error Distribution (50th percentile)')
...
def plot_coverage_bar(y_true, preds_dict):
...
def comprehensive_evaluation(
    ...
    if median_pred is not None:
        ...
        scatter_pred_vs_true(y_true_flat, median_pred.flatten(), 0.5)

    print("\n📌 3. Prediction Interval Width Over Time:")
    plot_prediction_interval_width(predictions)
    
    # --- Section 2: Deeper Quantile Analysis (NEW) ---
    print("\n📌 4. Prediction Intervals vs. True Values:")
    plot_prediction_intervals(y_true_flat, {q: p.flatten() for q, p in predictions.items()})

    print("\n📌 5. Quantile Coverage Calibration:")
    coverage_analysis(y_true_flat, {q: p.flatten() for q, p in predictions.items()})

    # --- Section 3: Final Metrics Table ---
    print("\n📌 6. Performance Metrics Table:")
    
    # Calculate metrics
    mae = mean_absolute_error(y_true_flat, median_pred.flatten()) if median_pred is not None else np.nan
    rmse = np.sqrt(mean_squared_error(y_true_flat, median_pred.flatten())) if median_pred is not None else np.nan
    
    pinball_losses = {f"Pinball Loss (q={q})": calculate_pinball_loss(y_true_flat, predictions[q].flatten(), q) for q in quantiles}
    avg_pinball = np.mean(list(pinball_losses.values()))
    
    lower_q = min(quantiles)
    upper_q = max(quantiles)
    coverage = np.mean((y_true_flat >= predictions[lower_q].flatten()) &
                       (y_true_flat <= predictions[upper_q].flatten())) * 100
    ...
    results_data = {
        "Model": model_name,
        "Total Training Time (s)": f"{training_time:.2f}" if training_time is not None else "N/A",
        "MAE (q=0.5)": mae,
        "RMSE (q=0.5)": rmse,
    ...
    display(results_df.set_index("Model").style.background_gradient(cmap="YlGnBu", axis=1))
    print("\n" + "="*80 + "\n")
