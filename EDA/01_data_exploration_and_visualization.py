...
def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_copy["bollinger_h"] = ta.volatility.BollingerBands(
        close=df_copy["close"]
    ).bollinger_hband()
    df_copy["bollinger_l"] = ta.volatility.BollingerBands(
        close=df_copy["close"]
    ).bollinger_lband()
...
def plot_price_and_indicators(df: pd.DataFrame, symbol: str):
    ...
    axes[0].fill_between(
        df["timestamp"], df["bollinger_l"], df["bollinger_h"], color="gray", alpha=0.1
    )
...
def plot_distributions_and_correlations(df: pd.DataFrame, symbol: str):
    ...
    sns.countplot(
        x="target", data=df, ax=axes[1], palette="viridis", hue="target", legend=False
    )
    ...
    sns.heatmap(
        corr_matrix, annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5, ax=axes[2]
    )
...
def explore_data_pipeline(symbol: str = "AAPL"):
    ...
    print(
        df_features[["close", "volume", "returns", "rsi", "macd", "target"]].describe()
    )
    print("------------------------------------------\n")
