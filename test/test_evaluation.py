...
def test_calculate_pinball_loss():
    ...
    assert calculate_pinball_loss(y_true, y_pred, quantile) == pytest.approx(expected_loss)
    ...
    expected_loss_2 = 1.5
    assert calculate_pinball_loss(y_true, y_pred, quantile_2) == pytest.approx(expected_loss_2)
...
def test_plotting_functions_run_without_error(mock_show, sample_y_true, sample_predictions, sample_history_dict):
    """
...
def test_comprehensive_evaluation_runs_successfully(mock_show, mock_display, capsys, sample_y_true, sample_predictions, sample_history_dict):
    """
    ...
    assert isinstance(call_args[0], pd.io.formats.style.Styler) # Check if a styled DataFrame was passed
...
@patch('matplotlib.pyplot.show')
def test_comprehensive_evaluation_no_median(mock_show, mock_display, capsys, sample_y_true, sample_predictions_no_median, sample_history_dict):
    """
