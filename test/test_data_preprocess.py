...
def test_output_shapes_and_types(dummy_data):
    ...
    (
        ...
        x_scaler,
        y_scaler,
    ) = preprocess_for_quantile_regression(
        dummy_data, lookback=lookback, future_horizon=future_horizon
    )
    ...
    assert (
        X_train.ndim == 3
    ), "X_train should be 3-dimensional (samples, lookback, features)"
    ...
    ), f"X_train lookback dimension should be {lookback}"
    assert X_train.shape[2] == 12, "There should be 12 features"
    assert (
        X_test.shape[1] == lookback
    ), f"X_test lookback dimension should be {lookback}"
    ...
    assert not np.array_equal(
        np.unique(y_train), np.array([0, 1])
    ), "Target should be continuous, not binary"
